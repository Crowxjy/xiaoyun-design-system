...
本文档旨在说明如何在项目中接入由 `@xiaoyun-ds/icons` npm 包提供的独立 SVG 图标库。

## 1. 核心概念：SVG 雪碧图 (Sprite)

我们使用**SVG 雪碧图 (Sprite)**技术将所有仓库内维护的图标打包到一个文件中。
它的优势在于：
1. **体积小、请求少**：所有图标在一个文件中，只需一次加载。
2. **支持 CSS 动态换色**：脚本会将非透明颜色替换成 `currentColor`，这意味着您可以像改变字体颜色一样通过 CSS 控制图标颜色（完美适配明暗主题切换）。
...
1. **查找图标名称**：
   ...
   - 图标名称由脚本根据当前仓库中的图标产物生成，并自动补齐线面属性（如 `-line`, `-fill`）。

2. **更新图标库**：
   - 当仓库中的图标雪碧图、注入脚本或预览页有更新时。
   - 在终端执行以下命令校验当前图标产物是否齐全：
     ```bash
     npm run sync-icons
     ```
