...
console.log('🚀 初始化 Xiaoyun Design System...');
...
async function installMissingDependencies() {
  const tokensPath = resolvePackagePath('@xiaoyun-ds/tokens', 'xiaoyun-ds-tokens.css');
  const iconsPath = resolvePackagePath('@xiaoyun-ds/icons', 'assets/sprite.svg');

  if (!tokensPath || !iconsPath) {
    console.log('📦 正在补全缺失的 @xiaoyun-ds/tokens 或 @xiaoyun-ds/icons 依赖...');
    try {
      execSync('npm install @xiaoyun-ds/tokens @xiaoyun-ds/icons --no-save', { stdio: 'inherit', cwd });
    } catch (error) {
...
async function main() {
  ...
  let tokensSrc = resolvePackagePath('@xiaoyun-ds/tokens', 'xiaoyun-ds-tokens.css');
  let iconsSrc = resolvePackagePath('@xiaoyun-ds/icons', 'assets/sprite.svg');
  
  if (!tokensSrc) {
    tokensSrc = path.join(__dirname, '../../tokens/xiaoyun-ds-tokens.css');
  }
  if (!iconsSrc) {
  ...
  await fs.copyFile(tokensSrc, path.join(stylesDestDir, 'xiaoyun-ds-tokens.css'));
  console.log(`   ✅ 提取: ${path.relative(cwd, path.join(stylesDestDir, 'xiaoyun-ds-tokens.css'))}`);
  ...
  console.log(`
  <link rel="stylesheet" href="./${path.relative(cwd, path.join(stylesDestDir, 'xiaoyun-ds-tokens.css'))}">
